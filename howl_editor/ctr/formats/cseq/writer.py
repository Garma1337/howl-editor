...
from howl_editor.ctr.formats.cseq.models import (
    CseqFile, CseqInstrument, CseqPercussion, CseqSong, CseqTrack, CseqEvent,
    CSEQ_EVENT_PARAMS,
)
...
class CseqWriter:
    ...
    def _reserve_song_offsets(self, out: bytearray, cseq: CseqFile) -> None:
        out += b"\x00" * (len(cseq.songs) * fmt.OFFSET_SIZE)

    def _pad_to_alignment(self, out: bytearray) -> None:
        remainder = len(out) % fmt.ALIGNMENT
        if remainder:
    ...
    def _write_song(self, out: bytearray, song: CseqSong) -> None:
        ...
        for i, t_off in enumerate(track_offsets):
            pack_into("<H", out, track_ptr_pos + i * fmt.OFFSET_SIZE, t_off)
