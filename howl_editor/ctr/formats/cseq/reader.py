...
from howl_editor.ctr.formats.cseq import format as fmt
from howl_editor.ctr.formats.cseq.models import (
    CseqFile, CseqInstrument, CseqPercussion, CseqInfo, CseqSong, CseqTrack,
    CseqEvent, CseqEventType, CSEQ_EVENT_PARAMS, CSEQ_TERMINAL_EVENTS
)
...
class CseqReader:
    ...
    def read(self, data: bytes) -> CseqFile:
        ...
        pos = self._align_to(pos, fmt.ALIGNMENT)
    ...
    def _parse_song(self, data: bytes, seq_start: int, offset: int) -> CseqSong:
        ...
        header_total = fmt.SONG_HEADER_SIZE + track_num * fmt.OFFSET_SIZE
        tracks_start = self._align_to(seq_start + offset + header_total, fmt.ALIGNMENT)
